...
- **A superseded attempt publishes nothing.** `#preprocess` begins the attempt (`Attempts.begin()`, which clears the current request) before preparation, so a completion of an earlier attempt is dropped even when the new attempt is blocked on a child. Do not move that reset after the request allocation of `Attempts.run()`.
- **Announcements never corrupt state.** `Attempts` records `processed`, `first` and the readiness promise before its `Announcer` announces `change` and `_notify`, and a subscriber or hook that throws is reported by the announcer, not propagated. Keep the announcement after the bookkeeping.
...
- Keep `dp.ts` within the 300-line target by extracting a collaborator, as `attempts.ts`, `announcer.ts`, `failure.ts`, `subscriptions.ts` and `types.ts` were; do not compress it. The implementation keeps every member the outer object exposes as a prototype accessor or method that delegates to its collaborators.
