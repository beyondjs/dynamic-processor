...
**Completion.** The response decides two flags. `undefined`, `true`, `false` and `null` set both `changed` and `notify` to their truthiness, with `undefined` meaning true; an object sets each one, defaulting to true. The completion records `tu`, clears `processing`, sets `processed`, resolves `ready`, clears `first`, then its [announcer](../src/modules/main/announcer.ts) announces: `change` is emitted when the result changed or on the first completion, and `_notify()` runs when `notify` is set, except on the first completion unless `notifyOnFirst`. A subscriber or a `_notify` that throws, or a `_notify` whose promise rejects, is reported by the announcer and alters nothing already recorded; the remaining subscribers of the Node emitter after a throwing one are not called, which is the emitter's behaviour. A processing that took more than two seconds is written to the log.
...
**Destruction.** `destroy()` releases the subscriptions to every child and the checkpoint timer, marks the object destroyed, leaves the registry, clears the request, resolves a pending `ready` and emits `change` once more. It does not destroy the children, does not remove the object's own subscribers and throws when called twice. Whoever created a child destroys it.
...
[beyond.json](../beyond.json) selects [src/package.json](../src/package.json), which declares the Node distributions and the public dependencies. The [module tsconfig](../src/modules/main/tsconfig.json) requires Node types. Of its two distributions, `node-ts` compiles with `tsc`, and that compilation is what writes the declaration of the public module, [main.d.ts](../src/modules/node_modules/@beyond-js/dynamic-processor/main.d.ts), under `src/modules/node_modules` for editors. The file is generated: regenerate it by serving this package through an Engine with that distribution, and never edit it. The sources must compile under that `tsc`, whose `Error` typings accept no options argument: `ProcessorFailure` defines its `cause` itself for that reason. The tests under [tests/](../tests/README.md) import the compiled public module; [validation](validation.md) is the contract-to-test matrix with what remains unverified.
