# dynamic-processor agent instructions
...
The mixin forwards the prototype members of the implementation only: a member the outer object must expose is a prototype accessor or method, never an instance field. `_events` is an accessor for that reason, since a subclass that emits an event of its own — `@beyond-js/finder` does — would otherwise read `undefined`. Readiness settles on every path (a failure rejects it with a `ProcessorFailure`, destruction resolves it), a superseded attempt publishes nothing, and announcements never corrupt the recorded state. [The lifecycle directory](src/modules/main/AGENTS.md) states these invariants; [validation](docs/validation.md) maps each to its test. Run the tests after any change to the lifecycle.
