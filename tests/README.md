...
`support.mjs` holds the helpers these files share: gates a hook awaits and the test opens, a turn of the event loop, and a wait for observed state. None of them waits for a length of time.

| File | Establishes |
| --- | --- |
| `lifecycle.test.mjs` | Readiness, first completion, invalidation, response flags, asynchronous and thenable processing, superseded attempts, registered and required children, hold reasons, shared children |
